...
| Evidence              | `meminit new STRAT` (and `meminit new PLAN`) returned `data.template.applied=false`, `source=none` and emitted a markerless skeleton. `init` scaffolded templates for only ADR/PRD/FDD (`src/meminit/core/assets/org_profiles/default/templates/`) while the default config defined ~20 `document_types`. |
| Closure evidence      | Closed on 2026-05-31 by adding first-class STRAT template coverage in repo config, init scaffolding, packaged assets, and resolver/new-document regression tests.                                                                                                                                    |
...
| Closure evidence      | Closed on 2026-05-31 by replacing the no-template fallback with a marker-aware skeleton containing agent prompts and parsed section metadata.                                   |
...
| Closure evidence      | Closed on 2026-05-31 by extending `migrate-ids` to detect canonical-looking prefix/type-segment mismatches, allocate replacement IDs from the active namespace, emit `ID_PREFIX_MISMATCH` advice, and rewrite references when requested.                                                                                                    |
