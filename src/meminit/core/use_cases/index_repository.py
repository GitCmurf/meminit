...
def _generate_kanban(
        ...
        if not col_entries:
            ...
            for entry in col_entries:
                ...
                status = sanitize_field(
                    entry.get("status", ""), max_length=None, html_escape=False
                )
                ...
                if notes_raw:
                    notes_sanitized = sanitize_field(
                        notes_raw, max_length=500, html_escape=False
                    )
                    if notes_sanitized:
    ...
    for col_name in _KANBAN_COLUMNS:
        ...
        lines.append(
            f'<section class="kanban-column kanban-{col_class}" aria-label="{col_name}">'
        )
        lines.append(
            f'<h3>{col_name} <span class="kanban-count">{len(col_entries)}</span></h3>'
        )
        ...
        for entry in col_entries:
            ...
            status_slug = (
                status_raw.lower().replace(" ", "-") if status_raw else "draft"
            )
            status_escaped = sanitize_html(status_raw)
    ...
    def execute(self) -> IndexBuildReport:
                        ...
                        if resolved_state is not None:
                            ...
                            if state_entry.notes is not None:
                                sanitized_notes = sanitize_field(
                                    state_entry.notes, max_length=500, html_escape=True
                                )
                                if sanitized_notes:
        ...
        if project_state:
            validation_issues = validate_project_state(
                project_state, known_doc_ids, self._root_dir
            )
