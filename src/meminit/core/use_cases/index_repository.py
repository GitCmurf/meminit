...
def _generate_catalog(
    ...
    headers = ["ID", "Title", "Type", "Doc Status", "Impl State", "Last Active", "Owner"]
    ...
    for group_name in _GROUP_ORDER:
        ...
        for entry in group_entries:
            ...
            row = [
                escape_markdown_table(entry.get("document_id", "")),
                escape_markdown_table(entry.get("title", "")),
                escape_markdown_table(entry.get("type", "")),
                escape_markdown_table(entry.get("status", "")),
                escape_markdown_table(entry.get("impl_state", "")),
                last_active,
...
def _generate_kanban(
    ...
    lines.append("")
    lines.append("> Use `meminit state set <ID> --impl-state \"<state>\"` to move items across the board.")
    lines.append("> See [Dashboard Documentation](../10-prd/prd-007-project-state-dashboard.md) or use `meminit state --help` for available commands.")
    lines.append("")
    lines.append(f"_Auto-generated by `meminit index`. Last built: {generated_at}._")
    lines.append("")
    ...
    for col_name in _KANBAN_COLUMNS:
        ...
        if not col_entries:
            ...
            for entry in col_entries:
                ...
                if notes_raw:
                    notes_sanitized = sanitize_field(notes_raw, max_length=500, html_escape=False)
                    if notes_sanitized:
    ...
    for col_name in _KANBAN_COLUMNS:
        ...
        lines.append(f'<h3>{col_name} <span class="kanban-count">{len(col_entries)}</span></h3>')
        ...
        for entry in col_entries:
            ...
            if rel_val:
                lines.append(f'<strong class="card-id"><a href="{sanitize_html(rel_val)}">{doc_id}</a></strong>')
            else:
...
class IndexRepositoryUseCase:
    def __init__(
        ...
        *,
        ...
        self._catalog_name = catalog_name or getattr(self._layout, "catalog_name", "catalog.md")
    ...
    def execute(self) -> IndexBuildReport:
        ...
        if self._output_catalog:
            catalog_out_name = self._catalog_name
