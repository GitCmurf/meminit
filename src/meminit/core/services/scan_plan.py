...
class PlanAction:
    ...
    def sort_key(self) -> tuple:
        ...
        try:
            action_enum = (
                self.action
                if isinstance(self.action, PlanActionType)
                else PlanActionType(self.action)
            )
...
class MigrationPlan:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "MigrationPlan":
        ...
        for idx, a_data in enumerate(data.get("actions", [])):
            ...
            safety = ActionSafety(
                destructive=bool(saf.get("destructive", False)),
                overwrites=bool(saf.get("overwrites", False)),
            )
