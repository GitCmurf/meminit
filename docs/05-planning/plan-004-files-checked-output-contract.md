...
- Roll `output_schema_version` to 2.0 for migrated `check` JSON responses while keeping non-migrated command outputs on their existing schema version.
...
- [x] Introduce command-scoped schema versioning in `src/meminit/core/services/output_contracts.py` (`OUTPUT_SCHEMA_VERSION: 1.0`, `CHECK_OUTPUT_SCHEMA_VERSION: 2.0`).
...
- [x] Keep non-migrated JSON outputs that embed `output_schema_version` on v1 until migrated contracts exist (including index generation output in `IndexRepositoryUseCase`).
...
- [x] Update index-related tests (`tests/core/use_cases/test_index_repository.py`, `tests/core/services/test_repo_layout.py`) for non-migrated v1.0 schema version consistency.
...
- [x] Regenerate `docs/01-indices/meminit.index.json` and keep its non-migrated schema version consistent with emitted payload shape.
...
- [x] Confirm `output_schema_version` is `2.0` for migrated `check` JSON outputs and remains schema-consistent for non-migrated command outputs and artifacts.
...
- [x] Specs updated (MEMINIT-SPEC-003 and MEMINIT-SPEC-004) and aligned with phased schema contracts (v2.0 for migrated `check`, v1.0 for non-migrated commands).
- [x] Code updated with new counters and new `files_checked` semantics.
- [x] Tests updated and passing, including schema validation.
- [x] Index artifact regenerated to v1.0 (non-migrated artifact contract).
