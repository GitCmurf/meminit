...
```
              ...
              └────────┬────────┘              └──────────┬──────────┘
                     ...
                     ┌────────────┼────────────┐
                     ▼            ▼            ▼
               catalog.md   catalog.json   kanban.md
...
(`catalog.md`) that includes governance status and implementation state
columns.
...
`--impl-state`), the generated `catalog.md` header MUST reflect the active
...
- `meminit state set <document_id> --impl-state <value>` — set or update
  a document's implementation state
- `meminit state set <document_id> --notes <text>` — set or update notes
...
- Markdown generation contract: required columns, sort order, composite
  grouping rules for `catalog.md`; timestamp display policy (see FR-3)
...
- Output path conventions: `docs/01-indices/catalog.md`,
...
| 3     | Table view         | `catalog.md` generation, sorting, grouping        |
...
3. `meminit index` generates a `catalog.md` table view sorted by activity
