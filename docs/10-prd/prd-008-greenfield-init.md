...
2. **Byte-invariance of governed documents (MEMINIT-STRAT-001, MEMINIT-GOV-001).** Bootstrap MUST NOT silently rewrite Approved governed documents. When a repo still contains the legacy `docs/00-governance/DocOps_Constitution.md`, `meminit init` MAY migrate it to the canonical `docs/00-governance/docops-constitution.md` to preserve the single GOV-001 invariant on rerun. Mutable per-asset state lives in `.meminit/setup.yaml`, never in governed frontmatter.
...
`manifest` MUST be the last installer in every plan. It writes `.meminit/setup.yaml` reflecting the actual outcome (post-apply hashes, not pre-apply intent). On `--dry-run`, it writes nothing and reports the manifest it _would_ have written.
...
```json
{
  "data": {
    ...
    "add_ons": [
      { "id": "security-first", "compatible_with": ["standard", "strict"] }
    ],
    "projections": [
      { "id": "codex", "source_assets": ["meminit-docops-skill"] }
    ]
  }
}
```
